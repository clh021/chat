...
build: generate
	@cd cmd; go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/chat"
	@echo "[OK] App binary was created!"

init: .git/hooks/pre-push .air

buildcross: generate
	@cd cmd; CGO_ENABLED=0 GOARCH=amd64 GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/chat.amd64"
	@cp ../bin/chat.amd64 ../bin/chat.x86_64
	@echo "[OK] App amd64 binary was created!"
	@cd cmd; CGO_ENABLED=0 GOARCH=arm64 GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/chat.arm64"
	@cp ../bin/chat.arm64 ../bin/chat.aarch64
	@echo "[OK] App arm64 binary was created!"
	@cd cmd; CGO_ENABLED=0 GOARCH=mips64le GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/chat.mips64le"
